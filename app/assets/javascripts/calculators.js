;(function($) {
    ...
    if($('#calc').data('type') != 'client') {
        ...
        //if ($('#order_tarif_name').val() != "Выберите платформу" && $('#order_platform_name').val() != "Выберите платформу") {
        //    setTarif();
        //
        ...
        if($('#service_agent_summa').val() && $('#service_order_summa').val()){

            var summa = parseFloat($('#service_order_summa').val()),
                dohod_agenta = parseFloat($('#service_agent_summa').val()),
                agent_rate = 0;

            if(summa > 0){
                agent_rate = floorFigure(((dohod_agenta/summa) * 100),2);
            }


          $('.value.agent_rate').text(agent_rate + "%");
        }

        if($('#service_mfo_summa').val() && $('#service_order_summa').val()){

            var summa = parseFloat($('#service_order_summa').val()),
                dohod_mfo = parseFloat($('#service_mfo_summa').val()),
                mfo_rate = 0;

            if(summa > 0){
                mfo_rate = floorFigure(((dohod_mfo/summa * 100)),2);
            }

            $('.value.mfo_rate').text(mfo_rate + "%");
        }
        ...
        $('#order_platform_name, #order_tarif_name').on('change', function () {
            ...
            if(!isSelectedTarif()){
                alert('Укажите платформу и тариф');
                return;
            }
        ...
        $('#service_agent_summa').on('keyup, change', function (e) {
            ...
            $('#order_base_rate').val(order_rate);
            //$('#order_summa').val(summa);
            $('#order_dogovor_summa').val(order_summa);
            $('#order_mfo_summa').val(floorFigure(mfo_summa, 2));
...
function isSelectedTarif(){
    return !!($('#order_tarif_name').val().length && $('#order_platform_name').val().length);
}
...
function setTarif(platform, type) {
    ...
    //показыаем поле, если есть признак персонального поля в тарифе
    ...
    if(platform == "ММВБ") {
        $('#number-mmvb').removeClass('hidden')
    }else{
        $('#number-mmvb').addClass('hidden')
    }
}
...
function Calculate(summa, base_rate, by_rate_flag, by_order_flag, by_summa_flag) {
    ...
    var by_rate = !!by_rate_flag,
        ...
        mfo_margin = window.mfo.config.mfo_margin,
        dop_mfo_rate = 1.5,
        dop_agent_rate = 0.5,
        dop_mfo_summa = 0,
        dop_agent_summa = 0,
